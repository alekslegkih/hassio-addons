...
Этот аддон — обёртка над официальным сервером [ntfy](https://ntfy.sh),  
предназначенная для удобного развёртывания локального сервера уведомлений  
в экосистеме Home Assistant.

> [!WARNING]
>Аддон не изменяет логику ntfy, не добавляет собственных API и не управляет конфигурацией сервера  
...
> При перезапуске новые настройки будут применены
---
...
```yaml
# По умолчанию создаётся следующий конфиг:
...
# Ограничения на сообщения (не увеличивайте, если используете push-уведомления на мобильных устройствах)
...
Аддон корректно работает за reverse proxy (Nginx, Traefik и др.).

Если сервер доступен из интернета, рекомендуется:

- ограничить или отключить Web UI на уровне proxy,
- явно разрешить только необходимые топики,
- блокировать все остальные запросы.

Пример ограниченного конфигурационного  [файла](https://github.com/alekslegkih/hassio-addons/tree/main/ntfy-server/conf/ntfy.conf)  Nginx  
приведён исключительно как опциональный пример.  
Использование данного подхода не является обязательным.

---

## Web UI

Web UI ntfy включён по умолчанию.

Если вы не хотите, чтобы интерфейс был доступен извне:

- отключите его на уровне reverse proxy,
- либо ограничьте доступ по IP или аутентификации.

> [!TIP]
> Аддон не управляет Web UI и не изменяет его поведение.
