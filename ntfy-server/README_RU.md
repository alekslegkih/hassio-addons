...
[English version](https://github.com/alekslegkih/hassio-addons/blob/main/ntfy-server/README.md)

Этот аддон — обёртка над официальным сервером [ntfy](https://ntfy.sh), предназначенная для удобного развёртывания
локального сервера уведомлений в экосистеме Home Assistant.

> [!WARNING]
> Аддон не изменяет логику ntfy, не добавляет собственных API и не управляет внутренней конфигурацией сервера.  
> Управление пользователями, доступами и топиками выполняется стандартными [средствами](ntfy_com_ru.md) ntfy.
