...
# Функции конфигурации из config.sh переносим сюда
load_config() {
    ...
    if [ ! -f "$USER_CONFIG" ]; then
        ...
        if [ -f "$DEFAULT_CONFIG" ]; then
            ...
            bashio::log.warning "Please edit settings.yaml and restart addon"
            return 2
        else
...
# Выполняем загрузку конфигурации при инициализации
load_config
CONFIG_EXIT_CODE=$?

if [ $CONFIG_EXIT_CODE -eq 2 ]; then
    bashio::log.warning "First run detected - addon will exit"
    exit 0
