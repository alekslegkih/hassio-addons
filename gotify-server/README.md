...
- создаёт каталог /config,
- создаёт файл конфигурации config.yml,
...
Конфигурация, база данных и загруженные изображения
хранятся в каталоге /config, который монтируется Home Assistant Supervisor.
...
- Поддерживаются официальные клиенты Gotify для Android и iOS
- Для подключения используется URL Home Assistant
- Приложения и токены настраиваются через Web UI
